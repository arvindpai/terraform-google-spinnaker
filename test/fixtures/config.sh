...
function set_region() {
  # memorystore not supported in southamerica-east1
  REGION_LIST=$(gcloud compute regions list --format="value(name)" | grep -v southamerica-east1)
...
function set_project() {
  ...
  for dir in ${FIXTURE_DIRS}; do
    TF_VARS_FILE="${dir}/terraform.tfvars"
    if ! grep 'project =' "${TF_VARS_FILE}" -q; then
      DEFAULT_project=$(jq '.project_id' <"${GOOGLE_APPLICATION_CREDENTIALS}")
      echo "project = ${DEFAULT_project}" >>"${TF_VARS_FILE}"
    fi
  done
}
